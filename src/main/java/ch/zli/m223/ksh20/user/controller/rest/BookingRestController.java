...
public class BookingRestController {
    ...
    @PostMapping("/add")
    ResponseEntity<?> addBooking(@RequestBody BookingInputDto bookingInput,
                                 @RequestHeader("Authorization") String header) {
        try {
            String token = header.split(" ")[0].trim();
    ...
    ResponseEntity<?> deleteBooking(Model model,
                       @PathVariable Long id,
                       @RequestHeader("Authorization") String header) {
        String token = header.split(" ")[0].trim();
        if (jwtUtils.getRoleFromJwtToken(token).equals("admin") ||
                jwtUtils.getIdFromJwtToken(token) == bookingService.getBookingById(id).getUserId()) {
